...
// #define MEM_BITMAP_BASE 0xc009a000 //位图在内存中的基址 
// #define K_HEAP_START 0xc0100000 //定义堆的地址 用于内核动态分配内存 
...
// struct pool kernel_pool, user_pool;  //生成物理内核内存池以及用户内存池
// struct virtual_addr kernel_vaddr;    //此结构用来给内核分配虚拟地址

// static void mem_pool_init(uint32_t all_mem){
//     put_str("mem_pool_init start\n");
//     uint32_t page_table_size = PG_SIZE * 256; //一个页目录表，255个页表
//     uint32_t used_mem = page_table_size + 0x100000; //底端1MB空间已被占用
//     uint32_t free_mem = all_mem - used_mem;
//     uint16_t all_free_pages = free_mem / PG_SIZE;
//     uint16_t kernel_free_pages = all_free_pages / 2;
//     uint16_t user_free_pages = all_free_pages - kernel_free_pages; //简化位图操作，余数不做处理。虽然会丢失部分内存，但不需进行越界判断

//     uint32_t kbm_length =  kernel_free_pages / 8;
//     uint32_t ubm_length =  user_free_pages / 8;

//     uint32_t kp_strat = used_mem; //内核内存池起始地址
//     uint32_t up_start = kp_strat + kernel_free_pages * PG_SIZE;//用户内存池起始地址

//     kernel_pool.phy_addr_start = kp_strat;
...
//    kernel_vaddr.vaddr_bitmap.btmp_bytes_len = kbm_length; //用于维护内核的虚拟地址，所以和内核内存池大小一致
//    //位图的数组指向一块未使用的内存，目前定位在用户内存池和内核内存池之外
...
//在pf表示的虚拟内存池中申请pg_cnt个虚拟页，若申请成功返回虚拟页的起始地址，失败则返回NULL
...
//原理：1.通过vaddr_get在虚拟内存池中申请虚拟地址；2.通过palloc在物理内存池中申请物理页；
...
//从物理内存池中申请pg_cnt页内存，成功则返回其虚拟地址，失败返回NULL
void* get_user_pages(uint32_t pg_cnt){
	lock_acpuire(&user_pool.lock);
